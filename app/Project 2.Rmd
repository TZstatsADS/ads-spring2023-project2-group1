...
```{r}
data <- read.csv(file = 'New_York_City.csv')
head(data)
```
```{r}
...
server <- function(input, output) {
  ...
  output$plot1=renderPlot({
    data = borough_data()
    ggplot(data, aes(x = factor(VIOLATION.DESCRIPTION)))+  #change grade to VIOLATION.DESCRIPTION
         geom_bar(width = 0.9) + coord_flip() +
         scale_x_discrete(labels = function(x) str_wrap(x, width = 2))
    })
  output$plot2=renderPlot({
    data = borough_data()
    ggplot(data, aes(x = factor(GRADE)))+  #change grade to VIOLATION.DESCRIPTION
         geom_bar(width = 0.9) + coord_flip() +
